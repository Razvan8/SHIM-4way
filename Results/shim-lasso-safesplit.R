...
## DO SAFE SPLIT
split<-split_data_safe(xxxx.all, y.all, specified_columns = unlist(range_psi), additional_percentage = 0.64, seed = 1 )
x.train<-split$X_train
x.test<-split$X_test
y.train<-split$y_train
y.test<-split$y_test
...
res_lasso_train<-irlasso.cb(X=x.train, Y=y.train, lambda=lambda, w.lambda=NULL, beta0=NULL,
...
my_shim<-SHIM_4way(X=x.train, y=y.train, beta_init = beta_hat, gamma_init = gamma_hat, delta_init = delta_hat, tau_init=tau_hat, l1=l1, l2=l2, l3=l3, l4=l4, scale = FALSE)
...
###THIS HAS ALL COEFFICIENTS
fitted<-my_shim$fit(X=x.train, y=y.train, lambda_beta = lambda_beta, lambda_gamma = lambda_gamma, lambda_delta = lambda_delta, lambda_tau=lambda_tau, w_beta = 1, 
                    w_gamma = 1, w_delta = 1, w_tau=1, tol=1e-2, compute_Q = Q_bern, intercept = interc_init, use_intercept = TRUE, bind_C = FALSE)


beta_train_shim<-fitted$beta_all ### THESE ARE  ALL THE BETAS IN THE GOOD ORDER!! #THESE CAN BE STORED IN CASE SOME ERROR HAPPENS AFTER
intercept_train<-fitted$intercept #The intercept on train
predictions_test_shim<-my_shim$predict(self=fitted, X_new=x.test ) ################# PREDICTIONS ON TEST SET
